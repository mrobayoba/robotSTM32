...
/* Standard values for the configurations */

/* 8.4.1 GPIOx_MODER (two bits per PIN) */
enum
{
	GPIO_MODE_IN = 0,
	GPIO_MODE_OUT,
...
/* 8.4.2 GPIOx_OTYPER (one bit per PIN) */

enum
{
	GPIO_OTYPER_PUSHPULL = 0,
	GPIO_OTYPER_OPENDRAIN
};

/* 8.4.3 GPIOx_OSPEEDR (two bits per PIN) */
...
/* 8.4.4 GPIOx_PUPDR (two bits per PIN) */
...
/* 8.4.5 GPIOx_IDR (one bit per PIN) - this register is to read the PIN current state */

/* 8.4.6 GPIOx_ODR (ONE BIT PER pin) - this register is to write the PIN state (1 o 0).
 * This register can be wrote and read from the software, but it not guarantee an 'atomic' writing,
 * due to  it is preferable to use the BSRR register
 */

/* Definition of the names of the PINS */
...
/* Definition of the alternative functions */
...
typedef struct
{
	...
	uint8_t GPIO_PinOutputType;			// Selects output type: push-pull and openDrain
...
void gpio_WritePin(GPIO_Handler_t *pPinHandler, uint8_t new_State);
