...
static void exti_select_edge(EXTI_Config_t *extiConfig){
	...
	if(extiConfig->edgeType == EXTI_FALLING_EDGE){
	...
	else if(extiConfig->edgeType == EXTI_RISING_AND_FALLING_EDGES){
		/* Rising Trigger selection register */
		EXTI->RTSR |= (0x1UL << extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber);
		/* Falling Trigger selection register */
		EXTI->FTSR |= (0x1UL << extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber);
...
static void exti_config_interrupt(EXTI_Config_t *extiConfig){
	/* 6.0 Activamos la interrupción del canal que estamos configurando */
		...
		EXTI->IMR |= (0x1UL <<extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber);
		...
		switch (extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber) {
		case 0: {
			__NVIC_SetPriority(EXTI0_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI0_IRQn);
			break;
		}

		case 1: {
			__NVIC_SetPriority(EXTI1_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI1_IRQn);
			break;
		}
		case 2: {
			__NVIC_SetPriority(EXTI2_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI2_IRQn);
			break;
		}
		case 3: {
			__NVIC_SetPriority(EXTI3_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI3_IRQn);

			break;
		}
		case 4: {
			__NVIC_SetPriority(EXTI4_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI4_IRQn);
			break;
		}
		case 5 ... 9: {
			__NVIC_SetPriority(EXTI9_5_IRQn, (uint32_t) (extiConfig->interruptPriority));
			__NVIC_EnableIRQ(EXTI9_5_IRQn);
			break;
		}
		case 10 ...15: {
			__NVIC_SetPriority(EXTI15_10_IRQn, (uint32_t) (extiConfig->interruptPriority));
...
void exti_config_newInterrupt(EXTI_Config_t *extiConfig, uint8_t newPriority){
	...
	extiConfig->interruptPriority	=	newPriority;
//
	__disable_irq();
	exti_config_interrupt(extiConfig);
	__enable_irq();
...
/* 
 * ISR de la interrupción canales 0 a 4 (Independientes)
 */
